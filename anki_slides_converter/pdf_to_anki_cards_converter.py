...
class PDFToAnkiCardsConverter:
    def __init__(self, pdf_filename: str, skip_first: int = 0, skip_last: int = 0, password: str = ""):
    ...
    def get_image_filename(self, page_index: int) -> str:
        return (
            self._pdf_filename.split(os.sep)[-1]
            + "_page_"
    ...
    def output_images_to_directory(self, output_directory: str, dpi: int = 200, thread_count: int = multiprocessing.cpu_count()) -> None:
        ...
        page_count = pdf2image.pdfinfo_from_path(pdf_path, userpw = self._password)["Pages"]
        first_page = self._skip_first
        last_page = page_count - skip_last
        images = pdf2image.convert_from_path(self._pdf_filename, dpi = dpi, first_page = first_page, last_page = last_page, thread_count = thread_count, userpw = self._password)
        for image_number, image in enumerate(images):
            image.save(
                os.path.join(output_directory, self.get_image_filename(image_number))
            )

    def output_cards_to_csv_file(self, csv_file: csv) -> None:
